...
var CALLBACKRATE = 0.1
let ORDER_IN_PROGRESS = false



async function long(asset, price, stopLoss) {
    ...
    var stopPrice = new Decimal(stopLoss)
    let shortOrder = await binance.futuresSell(symbol, positionAmt.toFixed(3), price = tpPrice.toFixed(1), params = { type: 'TRAILING_STOP_MARKET', callbackRate: CALLBACKRATE, activationPrice: tpPrice.toFixed(1), timeInForce: 'GTC' })
...
async function short(asset, price, stopLoss) {
    if (ORDER_IN_PROGRESS) return;
    ...
    let longOrder = await binance.futuresBuy(symbol, positionAmt.abs().toFixed(3), price = tpPrice.toFixed(1), params = { type: 'TRAILING_STOP_MARKET', callbackRate: CALLBACKRATE, activationPrice: tpPrice.toFixed(1), timeInForce: 'GTC' })
